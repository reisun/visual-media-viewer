...
| ← → | 前/次の画像（動画: ±10秒シーク） |
| ↑ / ↓ | 前/次の画像フォルダ |
| Shift+↑ / Shift+↓ | `title_root` 配下の前/次フォルダ unit へ移動（初期状態では ↑/↓ と同じ） |
| Delete | 現在ファイルの親フォルダを確認後に Windows ごみ箱へ移動 |
...
- `Shift+↑ / Shift+↓` はこの `title_root` を基準に sibling unit を移動します。`title_root` が現在フォルダのままなら通常の `↑ / ↓` と同じです

### フォルダ削除

- `Delete` は現在表示中ファイルの親フォルダを対象に確認ダイアログを開きます
- 削除は Windows のごみ箱移動のみで、恒久削除はしません
- 確定時は、削除対象 subtree の外側にある次のメディアフォルダを先に確定してから削除し、成功後にその移動先を開きます
